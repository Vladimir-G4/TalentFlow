...
const CompanyProfile = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col relative overflow-hidden bg-gray-50 text-black px-6">
      {/* Header */}
      <div className="pt-10 pb-6 z-10 text-black text-center">
        <h3 className="text-5xl font-bold">{company.name}</h3>
        <h6 className="text-lg text-gray-600">{company.sector}</h6>
      </div>

      {/* Navigation Bar */}
      <header className="flex justify-center items-center mb-6 z-10 relative">
        <div className="w-full md:w-2/3 flex items-center justify-center space-x-6">
      ...
      <main className="flex-grow z-10 relative">
        <section className="mb-6 text-center">
          <img src={company.photo} alt={company.name} className="w-32 h-32 rounded-full mx-auto mb-4 shadow-md" />
          <p className="mt-2 text-gray-700">{company.blurb}</p>
        </section>

        {/* Company Details */}
        <div className="p-8 bg-white rounded-lg shadow-lg">
          <h2 className={`text-4xl font-poppins-medium ${marckScript}`}>About {company.name}</h2>
          <p className="mt-2 text-gray-700">{company.blurb}</p>
          <div className="mt-4">
          ...
          <div className="mt-6">
            <a 
              ...
              className="bg-purple-600 text-white py-3 px-6 rounded-full transition duration-300 hover:bg-purple-700 shadow-lg"
              target="_blank"
        ...
        <section className="mt-8">
          <h3 className="text-3xl font-bold mb-4">Student Reviews</h3>
          {company.students.map((student, index) => (
            <div key={index} className="flex items-start mb-4 p-4 bg-gray-50 rounded-lg shadow-sm transition duration-300 hover:shadow-md">
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center mt-1">
                  ...
                  {[...Array(5)].map((_, starIndex) => (
                    <svg
                      key={starIndex}
                      className={`w-5 h-5 ${starIndex < student.starRating ? 'text-yellow-500' : 'text-gray-300'}`}
                      fill="currentColor"
                      viewBox="0 0 20 20"
                    >
                  ...
                  <span className="ml-2 text-gray-500 italic">{student.review}</span>
                </div>
              </div>
            </div>
          ))}
